  ...
  if (items.length === 0) {
    return (
      <div className="flex min-h-screen flex-col bg-background text-text-main pb-24">
        <header className="sticky top-0 z-30 flex items-center bg-surface px-4 py-3 border-b border-border shadow-xs">
        ...
        <div className="flex flex-1 flex-col items-center justify-center p-6 text-center">
          ...
          <p className="mb-6 text-xs text-text-muted max-w-xs">
      ...
      <main className="p-4 max-w-2xl mx-auto w-full space-y-4">
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex items-center gap-3 rounded-3xl bg-surface p-3 border border-border shadow-xs"
            >
              <img src={item.image} alt={item.name} className="h-20 w-20 rounded-2xl object-cover bg-surface-hover" />

              <div className="flex flex-1 flex-col py-1">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-xs font-bold text-text-main line-clamp-1 pr-2">{item.name}</h3>
                ...
                <div className="mt-2 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3 rounded-full bg-surface-hover px-2.5 py-1 border border-border">
                    <button
        ...
        <div className="rounded-3xl bg-surface p-4 border border-border shadow-xs space-y-2 text-xs">
          ...
          <div className="border-t border-border pt-2 flex justify-between text-sm font-extrabold text-text-main">
            <span>Нийт Төлөх Дүн</span>
            <span className="text-lg text-emerald-600">{getTotalPrice().toLocaleString()}₮</span>
          </div>
        </div>
      </main>

      {/* Checkout Sticky Bottom Bar */}
      <div className="fixed inset-x-0 bottom-16 z-30 bg-surface/95 backdrop-blur-md p-4 border-t border-border shadow-lg">
