...
import { ChefHat, Clock, Users, Plus, CheckCircle2, ShoppingBag, ArrowLeft, Layers, Sparkles } from 'lucide-react';

export function RecipeKitsScreen() {
  const navigate = useNavigate();
  const { addBundle, getTotalItems } = useCartStore();
  ...
  return (
    <div className="min-h-screen bg-background pb-28 text-text-main">
      ...
      <main className="max-w-4xl mx-auto px-4 pt-4">
        {/* Banner */}
        <div className="relative mb-6 overflow-hidden rounded-3xl bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-900 p-6 text-white shadow-xl border border-emerald-500/20">
          ...
          <h1 className="text-2xl font-black text-white mb-2">Хоолны Орцоо Бэлэн Багцаар Ав</h1>
        ...
        <div className="space-y-6">
          {RECIPE_BUNDLES.map((bundle) => {
            ...
            return (
              <div
                key={bundle.id}
                className="overflow-hidden rounded-3xl border border-border bg-surface shadow-xs transition-all hover:border-emerald-500/30"
              >
                {/* Bundle Header Image */}
                <div className="relative h-48 w-full bg-surface-hover">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 text-white">
                    ...
                    <p className="text-xs text-gray-200">{bundle.description}</p>
                  </div>
                </div>
                ...
                {bundle.instructions && (
                  <div className="p-4 bg-surface-hover/40 border-b border-border">
                    <h4 className="text-xs font-bold text-text-main mb-2">Хийх заавар</h4>
                    <div className="space-y-1 text-xs text-text-muted">
                      {bundle.instructions.map((inst, i) => (
                        <p key={i}>{inst}</p>
                      ))}
                    </div>
                  </div>
