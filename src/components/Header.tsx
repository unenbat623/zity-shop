...
import { Search, ShoppingCart, MapPin, Moon, Sun, RefreshCw, ChefHat, Database, ChevronDown, CheckCircle2 } from 'lucide-react';
...
export function Header({ onOpenOdooModal }: { onOpenOdooModal?: () => void }) {
  ...
  const { config, isSyncing } = useOdooStore();
  ...
  return (
    <header className="sticky top-0 z-30 bg-surface/90 px-4 pt-3 pb-3 backdrop-blur-md border-b border-border shadow-xs">
      {/* Top Banner Row: Brand, Connection Badges, Theme, Cart */}
      <div className="flex items-center justify-between mb-3">
        {/* Brand logo & tagline */}
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-400 text-white shadow-md shadow-emerald-500/20 font-bold text-xl">
            Z
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <span className="font-extrabold text-base tracking-tight text-text-main">ZITY DELGUUR</span>
            ...
            <p className="text-[11px] text-text-muted flex items-center gap-1 font-medium">
              <ChefHat className="h-3 w-3 text-emerald-500" /> Zity Chef & Odoo ERP Synced
            </p>
          </div>
        </div>

        {/* Action icons: Odoo Sync badge, Theme toggle, Cart */}
        <div className="flex items-center gap-2">
          {/* Odoo Status Badge */}
          <button
            onClick={onOpenOdooModal}
            className="hidden sm:flex items-center gap-1.5 rounded-full bg-surface-hover px-3 py-1.5 text-xs font-semibold text-text-main border border-border hover:bg-border transition-colors"
          ...
          <button
            onClick={toggleTheme}
            className="flex h-9 w-9 items-center justify-center rounded-full bg-surface-hover text-text-main transition-colors hover:bg-border border border-border"
          >
          ...
          <button
            ...
            className="relative flex h-9 w-9 items-center justify-center rounded-full bg-emerald-500 text-white transition-transform active:scale-95 shadow-md shadow-emerald-500/20"
          >
      ...
      <div className="flex items-center justify-between gap-2 mb-3 bg-surface-hover/70 rounded-xl px-3 py-2 border border-border text-xs">
