...
export function BottomNav() {
  ...
  const activeOrdersCount = useActiveOrderCount();
  ...
  return (
    ...
    >
      {/* 5 таб тогтмол өргөнтэй байвал 320px дэлгэц дээр хальж гардаг —
          боломжтой зайг тэнцүү хуваана */}
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className="relative flex min-w-0 flex-1 flex-col items-center justify-center rounded-2xl py-1"
            >
              <span
                className={`mb-0.5 flex h-8 w-11 shrink-0 items-center justify-center rounded-2xl transition-all ${
              ...
              >
                <span className="relative">
                  ...
                  {badge !== undefined && (
                    <span className="absolute -right-2 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-400 px-1 text-[10px] font-black text-slate-900">
                      {badge}
                    </span>
                  )}
                </span>
              ...
              <span
                className={`w-full truncate px-0.5 text-center text-[10px] font-bold leading-tight tracking-tight ${
